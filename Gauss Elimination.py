...
def gausselim(A):                                           # Gaussian Elimination Function that takes a matrix array as an argument
    ...
    for i in range(n-1):                                    # Forward elimination loop
        ...
        p = -1                                              # Set pivot to default value
        for k in range(i, n):                               # Loop to Find first pivot
            if A[k][i] != 0:                                # Checks if entry is non-zero
                p = k                                       # Updates pivot value if entry is non-zero
                break                                       # breaks nested loop 

        if p == -1:                                         # Checks if rows does not have a non zero entry (no unique solution)
        ...
        for j in range(i+1, n):                             # Elimination loop
            ...
            Mji = A[j][i] / A[i][i]                         # Mji varialble, multiplier to make entry 0
            for k in range(i, n+1):                         
                A[j][k] = A[j][k] - Mji * A[i][k]           # Updates row with new column entry 

    if A[n-1][n-1] == 0:                                    # Checks bottom right element, if 0, no unique solutions. 
        print("No unique solution exists")
        return None
                                
                                                            # Backwards Elimination step
    x = [0.0] * n                                           # initializes a vector with zeroes 
    ...
    for i in range(n-2, -1, -1):                                            # Backward substitution loop starting from 2nd to last to first row with backwards step size
        sum_ai = sum(A[i][j] * x[j] for j in range(i+1, n))                 # Calculates sum of known variables mulitplied by coefficients
...
if solution:                                                                # If unique solution, print values
...
print("Approximation of equation value:", eq1, eq2, eq3, eq4, "\n")
...
print("Approximation of equation value:" , eq1, eq2, eq3, eq4, eq5)        # Print values OF equations to see accuracy of approximation
